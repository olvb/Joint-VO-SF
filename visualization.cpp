...
void VO_SF::initializeSceneCamera()
{
	...
	//Grid (ground)
	opengl::CGridPlaneXY::Ptr ground = opengl::CGridPlaneXY::Create();
	scene->insert( ground );

	//Camera
	CPose3D rel_lenspose(0,-0.022,0,0,0,0);
	CBox::Ptr camera = CBox::Create(math::TPoint3D(-0.02,-0.1,-0.01),math::TPoint3D(0.02,0.1,0.01));
	camera->setPose(cam_pose + rel_lenspose);
	camera->setColor(0,1,0);
	scene->insert( camera );

	//Frustum
	opengl::CFrustum::Ptr FOV = opengl::CFrustum::Create(0.3f, 2.f, 57.3*fovh, 57.3*fovv, 1.f, true, false);
	...
	opengl::CPointCloud::Ptr points_lf = opengl::CPointCloud::Create();
    ...
    opengl::CVectorField3D::Ptr sf = opengl::CVectorField3D::Create();
...
void VO_SF::initializeSceneDatasets()
{
	...
	window.getDefaultViewport()->setCustomBackgroundColor(mrpt::img::TColorf(1,1,1));
	...
	opengl::CPointCloudColoured::Ptr seg_points = opengl::CPointCloudColoured::Create();
